...
from database.db import SessionLocal, AvisoAdopcion, Comuna, Foto, ContactarPor, Region, crear_aviso, Comentario, Nota
...
def evaluaciones():
    ...
    avisos_con_promedio = []
    for aviso in avisos:
        promedio = session.query(func.avg(Nota.nota))\
                         .filter(Nota.aviso_id == aviso.id)\
                         .scalar()
        avisos_con_promedio.append({
            'aviso': aviso,
            'promedio': round(promedio, 1) if promedio else None
        })
    
    session.close()
    return render_template("Evaluaciones.html", avisos_data=avisos_con_promedio)

@app.route("/api/notas/agregar", methods=["POST"])
def agregar_nota():
    data = request.json
    aviso_id = data.get("avisoId")
    nota = data.get("nota")
    
    # Validaciones
    if not aviso_id or not nota:
        return jsonify({
            "success": False,
            "message": "Debe proporcionar avisoId y nota",
            "nuevoPromedio": None
        }), 400
    ...
    if not isinstance(nota, int) or nota < 1 or nota > 7:
        return jsonify({
            "success": False,
            "message": "La nota debe ser un número entero entre 1 y 7",
            "nuevoPromedio": None
        }), 400
    ...
    if not aviso:
        ...
        return jsonify({
            "success": False,
            "message": f"El aviso con ID {aviso_id} no existe",
            "nuevoPromedio": None
        }), 400
    
    # Agregar la nota
    nueva_nota = Nota(aviso_id=aviso_id, nota=nota)
    session.add(nueva_nota)
    session.commit()
    
    # Calcular nuevo promedio
    promedio = session.query(func.avg(Nota.nota))\
                     .filter(Nota.aviso_id == aviso_id)\
                     .scalar()
    ...
    return jsonify({
        ...
        "nuevoPromedio": round(promedio, 1) if promedio else 0.0
    })
